...
def get_entity_mask(s, entidades):
    entity_mask = ['' for _ in range(len(s))]
    ent_ranges = {}
    labels_freqs = defaultdict(int)
    entidades.sort(key=lambda ent : len(ent[0]), reverse=True)
    for valor, label in entidades:
        ent_range = None
        
        # possiveis posicoes em que a entidade pode comecar
        possible_begins = [i for i in range(len(s)) if s[i] == valor[0]]
        # verificar se a entidade inteira está após o indice possível,
        # caso esteja então aquela é a posição verdadeira da entidade
        for i in possible_begins:
            ...
            for j in range(len(valor)):
                if s[i + j] == valor[j]:
                    # tudo certo, continua
                    num_match += 1
                    if valor[j] == valor[-1] and num_match == len(valor):
                        # chegou até o fim da entidade, então é o lugar certo
                        ent_range = (i, i+j)
                else:
                    # essa é a posição errada, testar o próximo possível começo
                    break
            if ent_range is not None:
                ent_ranges[label] = ent_range
                labels_freqs[label] += 1
                break  # já encontrou o lugar da entidade
